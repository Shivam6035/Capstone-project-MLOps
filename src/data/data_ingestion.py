...
def load_params(params_path: str) -> dict:
    """Load parameters from a YAML file. Create it if it doesn't exist."""
    try:
        # Check if the file exists; if not, create it with default values
        if not os.path.exists(params_path):
            logging.warning('File not found: %s. Creating a default configuration file.', params_path)
            default_params = {
                'data_ingestion': {
                    'test_size': 0.2
                }
            ...
            logging.info('Default %s created successfully.', params_path)
...
def main():
    try:
        ...
        #
        # df = s3.fetch_file_from_s3("data.csv")S
